...
params = {}
mode = "1"
VcNm = getvar("VacName") + " Power Mode"
...
if DE.changed_device_name == VcNm:
	if (DE.Devices[VcNm].s_value == "10"):
		mode = "1"
		DE.Log("Python: Eco Vacuum Mode: " + str(DE.Devices[VcNm].s_value))
	elif (DE.Devices[VcNm].s_value == "20"):
		mode = "0"
		DE.Log("Python: Normal Vacuum Mode: " + str(DE.Devices[VcNm].s_value))
	elif (DE.Devices[VcNm].s_value == "30"):
		mode = "2"
		DE.Log("Python: Max Vacuum Mode: " + str(DE.Devices[VcNm].s_value))
	else:
		DE.Log("Python: Unknown Vacuum Mode: " + str(DE.Devices[VcNm].s_value))
	...
	try:
		...
		if dev[0]['device']['connection_status'] == "Online":
			...
			try:
				...
				vacparameters = "/" + str(params['SET_Power_Mode']['property']['key']) + "/datapoints.json?env=field_production"
				header = "-H 'authorization: auth_token " + token + "'"
				...
				packet = '{"id":' + str(params['SET_Power_Mode']['property']['key']) + ',"datapoint":{"value":"' + mode + '"}}'
